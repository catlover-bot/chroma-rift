...
「影の映写室」は、必須の点光源装置、任意の歪んだ部屋、実際に移動する展示体と映写機、最後の防火幕を別の仕組みとして実装する。以下は幾何・入力・保存・描画の検査であり、人が錯視や怖さを感じた測定ではない。

| 対象 | 固定するもの | 明示操作で変えるもの | 証拠と限界 |
| --- | --- | --- | --- |
| 大きな影 | 同じ24頂点のコート輪郭、受光面、2窓、カメラ設定、窓の面積許容値 | 実レール上の点光源位置 | 原物体の22三角形を投影し、そのクリップ結果を不透明に描画。窓の面積判定も同じ結果を使う。影の大きさの変化は物理投影であり色彩立体視ではない |
| 歪んだ部屋 | 部屋の全頂点、同一geometry・world scaleの2展示物、FOV | 歩く視点、実点検窓・取っ手カバーの開閉 | 基準視線上に全室頂点を構成。正面と側面を同じ実Sceneで描く。行列一致や画像で知覚の強さを保証しない |
| 映写機 | 実音源位置、敵の実眼・身体・床・壁、有限の稼働条件 | 取っ手の回転と音源の稼働 | ゲーム内の聴覚信号は音量ゼロでも発生。音源の位置と遮蔽を使い、現在の有効な視覚を優先する。錯視ではない |
| 防火幕 | 同じ幕の幅・高さ・厚さ、実出口 | 明示した下降操作と、その後の歩行 | 受理、閉鎖、脱出を分離。表示・衝突・視線の遮蔽に同じ下降位置を使う。幕操作だけでクリアしない |

## 点光源の数値と表示

ローカル光源は `L(s)=(1.5s,1.1,1.2s)`、`s∈[-1,1]`。遮蔽物は `z=2`、受光面は `z=6`。平面への投影は `t=dot(n,Q-L)/dot(n,P-L)`、`S=L+t(P-L)` とし、有限値、非平行、`t>1`、受光範囲を検査する。不正な投影は成功にしない。

ワールドへの固定変換は倍率0.55、平行移動 `(-1.4,.65,0)`。コート輪郭の高さは0.539m。初期の未クリップ投影高さは1.617m、受光面の下端でクリップした表示範囲は1.4135m。レール全長は約2.113m。描画の光源、小さいコート、影は同じ定義を使い、カメラ、品質、音、怖さ、色設定から光学変数を変更しない。光源はこの装置だけのモデルで、室内全体の実照明を計算するものではない。

2窓は各0.26×0.30（ローカル単位）。初期状態では両窓の面積を100%遮蔽する。遮蔽率2%以下を許可する数値上の余裕を設け、2,001点の走査では成功範囲 `[-1,-.748]` と `[.492,1]` を得た。41位置、窓ごと65×49点の独立Three ray/triangle検査との最大遮蔽率差は約0.001036。これは製品の幾何許容値であり、知覚の測定値ではない。

窓の左右は正面から見た画面上の左右に対応する。光/影は明暗と丸/交差の印、および読み上げラベルで区別する。両窓が許可状態でも、指を離して「灯りを固定する」を押すまで戸は開かない。取消時は確定済みレール位置へ戻す。VoiceOverの増減も同じレール値を変更する。

影用の頂点領域はシーン生成時に確保し、レール値が変わった時だけ更新する。三角形は同じ不透明輪郭を分割したもので、半透明の重なりによる暗さを作らない。受光面と影・窓枠は深度を分離し、追加rendererや室内全体の動的shadow mapは使わない。

## 歪んだ部屋

基準眼 `(0,1.6,0)` を中心に、元の直方体の各頂点へ `t=5.2/(6-1.7x)` を適用する射影変換で、床・天井・前後左右の面を作る。後壁は `z=5.2+1.7x` となる。全頂点は元の基準視線上にあり、基準投影誤差は約3e-16以下。単に一方の物体を遠くへ置いただけの部屋ではない。

2展示物は同じ3部品のgeometryを共有し、実寸は0.22×0.82×0.22m、scaleは両方とも `(1,1,1)`。観察時に拡縮、差替え、billboard、カメラroll/FOVの変更を行わない。展示の内部へは入れず、周囲の水平な支持床から見る。

正面の基準点は `(-4,1.6,7)`。中間例は `(-4.3,1.6,7.45)`、側面例は `(-11.45,1.6,10.3)`。初稿の側面向きは壁しか見えなかったため、実Sceneを開いて歩廊内の向きと位置を調整した。幾何や物体の寸法は変えていない。開口、遮蔽、物体の投影範囲は [描画QA](qa-goal010/README.md) に記録する。

窓を開くこと、構造を調べてメモを記録すること、保守通路を開くことは別の状態。どちらの物体が大きく見えたかを問わず、全部見送った経路でも脱出できる。メモの比較は同じ定義を使う軽量な2D図で、追加のゲーム描画ルートを作らない。

## 検証の境界

原動画は指定の既知パスから読み、17枚の時系列フレームを実閲覧した。動画全編の連続再生、音の実聴、視線、初見所要時間、実FPSを測定したとは扱わない。個人の原録画はGitや外部サービスへ送らない。

新しい静止画・映像は実Three Scene、材質、controller、入力経路を使用するSoftware WebGLの証拠。合成HUDは実コンポーネントから作るが、iPhoneのYoga・タッチ・GPUの録画ではない。生成済みと実閲覧済みの範囲はQAの記録で区別する。

新章のnative Canvasを10回出入りし、各回1rendererと全資源の破棄を検査した。出口到達のrender/presentationを失敗させると、未提示のクリアと移動を巻き戻す。GL境界を置き換えた試験なので実機性能の証明ではない。

iPhoneの表示、両手操作、VoiceOver、快適さ、恐怖、影や奥行きの知覚、実音、FPS・フレーム時間分布・熱は未検証。[実機手順](IPHONE_VALIDATION.md)、[実装とコマンド結果](GOAL-010.md)、[出典](ILLUSION-SOURCES.md) を参照する。
