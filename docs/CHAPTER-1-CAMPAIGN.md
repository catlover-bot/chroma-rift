...
扉手前のQA視点では3枚とも全文字が画角内にある。完走ログ上のhandoff位置にcameraを置くと01と03は扉が近すぎて文字の左右が切れ、02は収まる。03の銘板を目線に近づけ、完全に画角外になる状態は解消した。これは手動camera・初期stage worldの静止画比較であり、実プレイのカメラ操作や端末の可読性ではない。
