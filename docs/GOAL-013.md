...
05制御室の人数表示では `0` の左下線が欠け、右下線が二重だった。実sceneの数字を修正し、自然成功のcontroller記録187 frameで02→01→00の切替順と全数字の線分座標を検査した。固定QA cameraの3画像を開き、欠け・二重線のない表示を確認した。自然成功・開け直し復旧の実scene動画を再収録した結果、player cameraから表示が直接見えない既存動画のSHA-256は同一だった。source hash、WebGL計測、近接画像・再現scriptを `docs/qa-goal013/README.md` に更新した。修正後の `npm run check` はlint・型検査・109スイート/1,124テスト・iOS通常exportを通過。公開用source map付きiOS/Android exportは1,576/1,575 source・各10 asset、5エリア同梱・開発画面/probe除外、first-party各197 source本文一致を確認した。iOS Threeは1 source/22参照でclass identity一致、実行時循環は3対象とも223 modules/684 edges/0、Stage定義2件も通過。近接QAはnative HUD・iPhoneでの視認性の証明ではなく、公開ゲートは維持する。
...
05の出口録画を再点検し、扉の先が黒一色で、屋外操作の汎用四角形だけが浮いていることを発見した。出口のworld照準・commandは保ったまま、その汎用meshを描画対象から外し、空、遠景、地面、歩道、灯具を出口から見える位置へ置いた。自然成功と開け直し復旧を実controller/StageSceneで再実行し、最後の歩行と退館操作が双方で完了した。自然経路のframe 00170は出口手前、00186は屋外到達後で、空と歩道が連続して見える。復旧経路の最終frame 00368も屋外を描く。scene内の屋外背景と汎用マーカー除外をQAで検査し、両動画の全frameデコード、tool/video/110 source hash、WebGLの資源解放・error 0を確認した。最新の動画と抽出画像、hashは `docs/qa-goal013/README.md`。これはSoftware WebGLであり、native iPhoneの視認性や屋外の知覚評価には代わらない。
