...
終端扉画像の再点検では、先の01/02の扉面と外枠が近接cameraで判別しにくく、03も記号が主に見えた。同じworld床上cameraで見える位置に3sceneの内側の扉面・枠・取っ手を加え、390×844画像を再生成して開いた。画像上で3つとも扉の輪郭と取っ手を確認し、QA scriptに部品の視野内位置と扉面の色差を追加した。world、当たり判定、出口command、保存形式には変更がない。銘板に読める行き先名は描いておらず、正確な行き先案内と実機視認性はnative previewで残る。更新画像・reportは `docs/qa-goal013/`。
