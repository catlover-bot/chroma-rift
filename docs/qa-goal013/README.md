...
04/05の鍵の表示・動作修正後、両経路をCPU計測付きで再実行した。Node v24.20.0上の実controller更新とscene callback（60Hz）を `performance.now()` で測り、ChromiumのSwiftShader Software WebGLで反射を含む `renderer.render` のJS呼出し時間を別に測った。描画投入時間にはscreenshot取得、RAF待機、native presentation、GPU完了を含めない。数値はこのローカル環境の計測で、iPhoneのframe timeやFPSではない。
