...
04単独の記録： [31.1秒の動画](mirror-natural.mp4)、[1秒ごとの接触シート](mirror-natural-contact.png)、[固定した横顔と鍵形](mirror-figure-key.png)、[鍵が持ち上がる途中](mirror-key-lifting.png)、[鍵取得後](mirror-key-taken.png)、[作業中の鏡内の巡回体](mirror-actor-reflected.png)、[開いた格子の通路](mirror-three-ratchets.png)、[イベント・source hash](mirror-natural-report.json)、[WebGL計測](mirror-natural-webgl.json)。`node scripts/qa-mirror-natural.cjs` は新規04の実controllerと地形で中央の鍵、実鏡面の明示観察、練習、保持二回、途中の退避、格子通過まで歩く。怖さは控えめ。`StageScene` の一つの巡回体meshと実 `planarMirror.ts` をブラウザーで同じsceneへ載せ、反射→主passの順で描いた。native R3F/EXGLのframe ownerやHUD/audioはこの動画では動かしていない。字幕帯はQAラベル。390×844、10fps、311枚、simulation28.28秒、反射target 384×384。反射は311枚中134枚で実行され、画面外では省略、視野へ戻った最初のframeに更新。最大主pass50 calls、反射pass42 calls、3,579 triangles、終了時geometries/textures 0。これらはSoftware WebGLであり、iPhone FPS/発熱の測定ではない。動画SHA-256 `69697ad9a717a9c5e05e338fe07f47b8c02cbfc1cb43396ce86b3d415efd0e10`、接触シート `03d735ca1bfbcfdd51882f9db5879abbc69d648e55e2ce44bcf57cb29d7ee1b9`。実鏡面の観察はsimulation 7.95秒に実controllerで受理され、最終sessionのmirrorInspected=trueを確認した。固定した二つの横顔と鍵の輪郭を冒頭で確認し、取得操作後の0.3秒で鍵形が台板から浮いて消える。新しい画像とscene animationの可視・位置bitを照合した。05の制御盤も同じShapeGeometryから生成する。接触シートと抽出frameを開いて確認したが、MP4を連続視聴したとは記録しない。

接触シートは `node scripts/qa-mirror-natural.cjs` が生成したPNGフレームから、`ffmpeg -framerate 10 -i .expo/goal013/mirror-natural/frames/%05d.png -vf 'select=not(mod(n\,10)),scale=195:422,tile=6x6' -frames:v 1 .expo/goal013/mirror-natural/contact.png` で作った。鍵の前・移動中・消失後はframe 00000/00013/00016、鏡内の身体は00103、開いた格子の通路は00200を抽出した。鍵3枚のPNG SHA-256は `41431bcc7779ae82d389b4971cb36399db1b73309cb00c18ec129018f5f8f01f` / `f41b7f5edc60a7feca9cf8fa126f9d2c7660cd518b52f9bc8e37fd24c260aa47` / `93b765af5f748bae5189c9f8e378f0ee652fada79f06dd5eff649d289bfe8aff`。
...
| --- | ---: | ---: | ---: |
| 04 鏡廊 | 0.037 / 0.150 / 2.452 ms | 0.5 / 1.2 / 40.6 ms | 1,697 / 311 |
