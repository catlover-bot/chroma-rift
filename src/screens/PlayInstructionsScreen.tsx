...
export function PlayInstructionsScreen({ controls, onStart, onBack, onSettings, chapterId, horrorIntensity = 'standard', onHorrorChange }: {
  ...
  const simple=stageDefinition(chapterId)?.renderKind==='simple'?stageDefinition(chapterId):undefined;
  return (
    <Screen>
      <Heading>準備できました</Heading>
      {theatre ? <Body>影の映写室</Body> : vault ? <Body>測れない収蔵庫</Body> : gallery ? <Body>閉館後の展示室</Body> : simple ? <Body>{simple.title}</Body> : null}
      <Panel>
      ...
      <ActionButton label={theatre ? '映写室へ入る' : vault ? '収蔵庫へ入る' : gallery ? '展示室へ入る' : simple ? `${simple.title}へ入る` : '迷宮へ入る'} variant="primary" onPress={onStart} />
