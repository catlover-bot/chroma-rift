...
export function PlayInstructionsScreen({ controls, onStart, onBack, chapterId, horrorIntensity = 'standard', onHorrorChange }: {
  ...
  return (
    <Screen>
      ...
      {gallery ? <Body>閉館後の展示室</Body> : null}
      <Panel>
      ...
      <Body>{gallery ? 'まず出口を探そう。大きな非常灯スイッチは、近づいて押せます。' : '壁の紋章に近づいて調べよう。'}</Body>
      {gallery ? <Panel>
      ...
      <Body muted>{gallery ? '一時停止から操作と怖さを変更できます。' : '一時停止からドラッグ操作・ボタン操作を選べます。'}</Body>
      <ActionButton label={gallery ? '展示室へ入る' : '迷宮へ入る'} variant="primary" onPress={onStart} />
