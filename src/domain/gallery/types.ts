...
export type GalleryDevice = GalleryPuzzle | 'wiring';
export type DiscoveryId = 'chromatic' | 'shadow' | 'contour' | 'mask' | 'wiring' | 'hybrid' | 'shepard';
export type WiringControl = 'line' | 'cover';
export type WiringCheckpoint = { offset: number; cover: number; inspected: boolean; solved: boolean; attempts: number; compatibleBypass: boolean };
...
export type GalleryV2Progress = Omit<LegacyGalleryProgress, 'schemaVersion'> & {
...
export type GalleryProgress = Omit<GalleryV2Progress, 'schemaVersion' | 'story'> & {
  schemaVersion: 3; story: GalleryV2Progress['story'] & { crossingStarted: boolean; crossingPresented: boolean }; wiring: WiringCheckpoint; discoveries: Record<DiscoveryId, boolean>;
  maskWindowOpen: boolean; finalDoorClosed: boolean; completedFromV2: boolean;
};
export type GalleryDrag = { kind: 'shadow'; sampleId: SampleId; pointerId: number; point: Point2; offset: Point2 } |
  ...
  { kind: 'wiring'; control: WiringControl; pointerId: number; startPoint: Point2; startValue: number };
...
export type GalleryTransient = {
  actor: GalleryActor; lastSafePose: PlayerPose;
  sessionId: string; lastSeq: number; lastNowMs: number; mode: 'explore' | GalleryDevice;
  wiringOffset: number; wiringCover: number; wiringDoorOpen: number; exitClosureSeconds: number;
  ...
  lastDeviceResult?: { puzzle: GalleryDevice; correct: boolean } | undefined;
  feedback?: { puzzle: GalleryDevice; correct: boolean; sequence: number; remainingSeconds: number } | undefined;
};
export type GalleryAction =
  | { type: 'enter'; puzzle: GalleryDevice }
  | { type: 'take-power'; puzzle: GalleryPuzzle }
  | { type: 'light-on' | 'inspect-exit' | 'connect-power' | 'open-exit' | 'chromatic-compare' | 'mask-inspect' | 'mask-window' | 'hybrid-inspect' | 'close-exit' }
  ...
  | { type: 'wiring-start'; control: WiringControl; pointerId: number; point: Point2 }
  | { type: 'wiring-move'; pointerId: number; point: Point2 }
  | { type: 'wiring-end'; pointerId: number; inside: boolean }
  | { type: 'wiring-adjust'; control: WiringControl; delta: number }
  | { type: 'wiring-commit' };
export type GalleryCommand = { sessionId: string; seq: number; nowMs: number; action: GalleryAction };
export type GalleryEffect = { type: 'power-taken'; puzzle: GalleryPuzzle; sequence: number } | { type: 'power-connected' | 'light-on' | 'exit-opened' | 'exit-closed' | 'wiring-released'; sequence: number } | { type: 'gallery-released'; puzzle: GalleryPuzzle; sequence: number } | { type: 'message'; text: string } | { type: 'stop-input' } | { type: 'manipulated'; puzzle: GalleryDevice } | { type: 'discovered'; id: DiscoveryId };
