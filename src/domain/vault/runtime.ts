...
export function createVaultRuntime(checkpoint?: CheckpointState, session?: number, seed = VAULT_SEED): ChapterRuntime {
  ...
  const raw = accepted?.progress.vault ?? initialVaultProgress(seed);
  const saved = { ...raw, length: { ...raw.length }, rod: { ...raw.rod }, discoveries: { ...raw.discoveries }, aids: { ...raw.aids }, story: { ...raw.story } };
  const base = createBaseRuntime(VAULT_CHAPTER_ID, VAULT_SPAWN, undefined, session);
