...
import { RATCHET_COUNT, gatePassable, type TargetId } from './definition';
...
export function selectMirrorPresentation(session: StageSession) {
  return { objective: session.cleared ? '制御室への前室に着いた。' : !session.keyTaken ? '横顔の余白から隔離キーを取る。' :
    !session.practiced ? '前室の練習レバーを一度保持する。' : session.ratchets < RATCHET_COUNT ? `本レバーで格子を巻き上げる。歯止め ${session.ratchets}/3。` : gatePassable(session.gateLift) ? '開いた格子の先へ進む。' : '格子が上がっている。',
  hint: { text: session.ratchets === RATCHET_COUNT ? gatePassable(session.gateLift) ? '三つの歯止めは固定済み。保持せず、開いた格子を通れる。' : '三つの歯止めが固定し、格子が上がっている。' :
    !session.keyTaken ? '向き合う横顔の間に、同じ輪郭のキーがある。' : !session.practiced ? '低い台の短い取っ手が練習用。本機へ進む前に一度試す。' :
      '確定した歯止めは残る。危険なら手を離し、棚の陰へ退く。' },
  feedbackRevision: [session.keyTaken, session.practiced, session.ratchets, gatePassable(session.gateLift), session.cleared].join(':') };
}
...
export function selectMirrorAction(session: StageSession, targetId: TargetId): StageTargetPresentation {
  ...
  switch (targetId) {
    ...
    case 'mirror-corridor-winch':
      if (session.ratchets === RATCHET_COUNT) return result('completed', gatePassable(session.gateLift) ? '格子は開放済み' : '格子が上がっている', '三つの歯止めが固定した。格子の先へ歩いて進もう。');
    ...
    case 'mirror-corridor-exit': return session.cleared ? result('completed', '前室に到着済み', '制御室への前室に着いた。') :
      result('locked', '開いた格子を通る', '三つの歯止めを固定し、格子の先の扉まで歩く。');
  }
}
