...
import { createCheckpoint, MOVE_SPEED, projectWithCamera, type Vec3 } from '../../firstPerson';
import { createContourSpec, createShadowSpec, GALLERY_CONTOUR_FIXTURE, GALLERY_EXIT_PANEL_FIXTURE,
  GALLERY_LIGHT_FIXTURE, GALLERY_SHADOW_FIXTURE, GALLERY_WIRING_FIXTURE, normalizeAngle } from '../../gallery';
import { VAULT_EXIT_FIXTURE, VAULT_PARTITION_FIXTURE, vaultFixture } from '../../vault/definition';
import { LENGTH_SPEC, ROD_SPEC } from '../../vault/specs';
import { THEATRE_CURTAIN_FIXTURE, THEATRE_LIGHT_FIXTURE } from '../../theatre/definition';
import { lightHandlePoint } from '../../theatre/lightGate';
import { actorFullyContained, doorSweepClear } from '../../stages/departure-control-v1/definition';
import { isStageSession as isControlSession } from '../../stages/departure-control-v1/session';
import { isStageSession as isMirrorSession } from '../../stages/mirror-corridor-v1/session';
import { advanceController, beginStageHoldController, commandController, controllerSnapshot,
  createController, endStageHoldController, interactController, syncCamera, type RuntimeController } from '../../../rendering/firstPerson/runtimeController';
import { galleryAction } from '../../../rendering/firstPerson/galleryController';
import { vaultAction, vaultPointer } from '../../../rendering/firstPerson/vaultController';
import { theatreAction, theatrePointer } from '../../../rendering/firstPerson/theatreController';
...
import { completeCampaignArea, createChapterOneSession, recordCampaignCheckpoint, type ChapterOneSession } from '../session';
...
type Run = { controller: RuntimeController; camera: THREE.PerspectiveCamera };
function open(session: ChapterOneSession, intensity: 'standard' | 'subdued'): Run {
...
test.each([
  ...
  for (let index = 0; index < CHAPTER_ONE.areas.length; index++) {
    ...
    const run = open(session, intensity);
    expect(run.controller.runtime.chapterId).toBe(area.stageId);
    if (index === 0) finishGallery(run, order);
    else if (index < 4) finish[index - 1]!(run);
    else finishControl(run, () => {
    ...
    const cleared = createCheckpoint(run.controller.runtime);
