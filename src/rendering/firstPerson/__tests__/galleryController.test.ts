...
describe('gallery logical touch points and actual Three camera (presentation contract substituted)', () => {
  ...
  it('rejects stale sessions, background input and retired callbacks, and sends actual movement only on a presented frame', () => {
    ...
    const detach = attachControllerAudio(c, { event: jest.fn(), dispose, setActive: jest.fn(), recover: jest.fn().mockResolvedValue(true), setPreviewActive: jest.fn(), setMusicState: jest.fn(), advanceMusic: jest.fn(), duckMusic: jest.fn(), setEnvironment: jest.fn(), playIllusion: jest.fn(), stopIllusion: jest.fn(), beginEnding: jest.fn(), updatePreferences: jest.fn(), movement, actorMovement: jest.fn(), stopMovement: jest.fn(), setListenerPosition: jest.fn(), whenReady: jest.fn().mockResolvedValue(undefined), getDiagnostics: jest.fn() });
