...
const galleryOnly=process.argv.includes('--gallery-only'),chapterOne=process.argv.includes('--chapter-one'),storyLayout=process.argv.includes('--story-layout'),sceneReplay=process.argv.includes('--scene-replay');
...
async function extractChapterOne(){
 ...
 const frames=[],hudTrees=[],hudMap=new Map(),events=[],route=[],motionSamples=[],motionAreas=[],storyScreens=[],replaySamples=[];
 ...
 for(let index=0;index<CHAPTER_ONE.areas.length;index++){
  ...
  const motionSample=phase=>{
   ...
   if(sceneReplay&&(phase!=='simulation'||simulationTicks%12===0))
    replaySamples.push({area:area.id,stageId:area.stageId,tick:simulationTicks,
     simulationSeconds:Number(simulationSeconds.toFixed(6)),phase,
 ...
 if(sceneReplay)fs.writeFileSync(path.join(out,'scene-replay-runtimes.json'),JSON.stringify({method:'Deep-copied runtime states from one actual App host and its five natural mounted controllers; every twelfth simulation update plus entry and clear. Scene rendering happens separately and is not the native App Canvas.',runId,samples:replaySamples},null,2)+'\n');
 ...
 const report={method:'One actual App host mount, five mounted FirstPersonScreen controllers, natural domain routes and serialized campaign transitions. Entry/cleared scene+HUD stills come from each mounted App controller; a separate 10Hz-style per-six-update motion log covers intervening domain simulation but not scene rendering. The ending is a separate App still. This is not a continuous video of play. The Three JSON snapshot omits the live planar-mirror render target; the separate mirror QA checks the reflection. AsyncStorage is isolated memory; native Canvas/ready/audio are stubbed and the React Three scene host does not reconcile props. Browser Software WebGL/CSS, not native Yoga/EXGL, touch, sound, perception or iPhone FPS.',
  substitutions:{storage:'Isolated in-memory AsyncStorage; real application codec/writer paths.',
   nativeCanvas:'Stub ready/owner lifecycle and camera; no EXGL presentation or native gesture recognition.',
   audio:'Audio preference disabled and unavailable no-op owner; this route does not prove audio playback or App audio ownership.',
   gateCallbacks:'QA manually calls onValidatedEntry, then onCheckpoint and onComplete with the actual naturally cleared controller checkpoint. Completion callback timing and accepted native presentation are not exercised by this helper.',
   ending:'Actual App intro and credits trees; credits opened by the actual button. Snapshot timing is authored, not the native 12-second afterglow timer.'},
...
async function main(){if((storyLayout||sceneReplay)&&!chapterOne)throw Error('--story-layout and --scene-replay require --chapter-one');if(sceneReplay&&!process.argv.includes('--extract-only'))throw Error('--scene-replay requires --extract-only');if(chapterOne&&!process.argv.includes('--sample-only')&&!process.argv.includes('--extract-only'))throw Error('Chapter-one App route supports snapshot capture only; use --sample-only');const report=process.argv.includes('--capture-only')?JSON.parse(fs.readFileSync(path.join(out,'report.json'))):chapterOne?await extractChapterOne():await extract();if(process.argv.includes('--capture-only'))for(const[f,hash]of Object.entries(report.sourceHashes))if(sha256(fs.readFileSync(path.join(source,f)))!==hash)throw Error('Source changed since extraction: '+f);if(!process.argv.includes('--extract-only'))await capture(report);bridge.verify();console.log(JSON.stringify({frames:report.frames,duration:report.duration,entries:report.owners.length,out}));}
