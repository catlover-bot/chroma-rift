...
const viewport=option('viewport')||'390x844',fontScale=Number(option('font-scale')||1.5);
if(!['320x568','390x844','430x932'].includes(viewport)||!Number.isFinite(fontScale)||fontScale<1||fontScale>2)
  throw Error('viewport must be 320x568, 390x844 or 430x932; font-scale must be 1..2');
...
const bridge=installSourceBridge(source),context={width,height,fontScale,bindController:false},native=installNativeHudBridge(context);
...
async function extract(){
 ...
 json('animation.json',{width,height,fontScale,camera:camera.uuid,hudTrees,frames});json('timeline.json',report);bridge.verify();json('source-hashes.json',Object.fromEntries(bridge.hashes));restoreTimers();return report;
}
function viewer(){
...
const r=new THREE.WebGLRenderer({antialias:true,preserveDrawingBuffer:true});r.setPixelRatio(1);r.setSize(${width},${height});r.outputColorSpace=THREE.SRGBColorSpace;r.toneMapping=THREE.NoToneMapping;
...
window.hudAudit=()=>auditHUD(root,null);
window.navigationAudit=()=>auditNavigationHUD(root);
window.deviceLayout=()=>{const play=root.querySelector('[data-testid="first-person-play"]'),scroll=root.querySelector('[data-testid="theatre-device-scroll"]');return{screen:hudRect(root),play:play?hudRect(play):null,scroll:scroll?{...hudRect(scroll),scrollHeight:scroll.scrollHeight,clientHeight:scroll.clientHeight}:null};};
window.scrollDevice=toEnd=>{const scroll=root.querySelector('[data-testid="theatre-device-scroll"]');if(scroll)scroll.scrollTop=toEnd?scroll.scrollHeight:0;};
...
async function capture(report){
 ...
 try{
  ...
  const captureFrames=process.argv.includes('--sample-only')?[...new Set([0,Math.floor(report.frames*.1),Math.floor(report.frames*.25),Math.floor(report.frames*.5),Math.floor(report.frames*.7),Math.floor(report.frames*.75),report.frames-1])]:Array.from({length:Math.ceil(report.frames/(30/captureFps))},(_,i)=>i*(30/captureFps));const hudAudits=[];for(const [index,frame] of captureFrames.entries()){const stats=await browser.evaluate('window.draw('+frame+')');maxCalls=Math.max(maxCalls,stats.calls);maxTriangles=Math.max(maxTriangles,stats.triangles);const stem=String(process.argv.includes('--sample-only')?frame:index).padStart(6,'0');const shot=await browser.send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});fs.writeFileSync(path.join(frames,stem+'.png'),Buffer.from(shot.data,'base64'));if(process.argv.includes('--sample-only')){const record={frame,hud:await browser.evaluate('window.hudAudit()'),navigation:await browser.evaluate('window.navigationAudit()'),layout:await browser.evaluate('window.deviceLayout()')};if(record.layout.scroll&&record.layout.scroll.scrollHeight>record.layout.scroll.clientHeight){await browser.evaluate('window.scrollDevice(true)');await browser.evaluate('new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)))');const bottom=await browser.send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});fs.writeFileSync(path.join(frames,stem+'-bottom.png'),Buffer.from(bottom.data,'base64'));record.scrolled={hud:await browser.evaluate('window.hudAudit()'),layout:await browser.evaluate('window.deviceLayout()')};await browser.evaluate('window.scrollDevice(false)');}hudAudits.push(record);}if(index%50===0)console.log('captured '+index+'/'+captureFrames.length);}if(hudAudits.length)json('hud-audit.json',{boundary:'React Native host tree translated to browser CSS; not Yoga or native text metrics',viewport,fontScale,frames:hudAudits});
  if(process.argv.includes('--sample-only')){const overlaps=hudAudits.filter(record=>['pauseOverlapsObjective','contextOverlapsObjective','contextOverlapsMovement','noticeOverlapsObjective','noticeOverlapsContext','contextOverlapsAction'].some(key=>record.navigation[key]));if(overlaps.length)throw Error('Exploration HUD overlap in sampled frames: '+overlaps.map(record=>record.frame).join(','));}
  if(process.argv.includes('--sample-only')&&scenario==='light'){const reachable=label=>hudAudits.some(record=>[record.hud,record.scrolled?.hud].filter(Boolean).some(hud=>hud.buttons.some(button=>button.label===label&&!button.disabled&&button.visible.bottom-button.visible.top>=44&&button.visible.right-button.visible.left>=44)));if(!reachable(commitLabel)||!reachable(solvedLeaveLabel))throw Error('Reachable light commit/leave button missing in sampled HUD');}
