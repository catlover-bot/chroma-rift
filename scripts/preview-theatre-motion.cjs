...
const root=path.resolve(__dirname,'..'),scenario=process.argv.find(a=>a.startsWith('--scenario='))?.slice(11)||'light',out=path.join(root,'.expo/goal010',scenario==='light'?'light-motion':'motion-'+scenario);
if(!['light','route-east','route-inspect','projector','projector-control','capture-retry','curtain-portrait'].includes(scenario))throw Error('Unknown scenario');
fs.mkdirSync(out,{recursive:true});
const bridge=installSourceBridge(root),context={width:390,height:844,fontScale:1.5,bindController:false},native=installNativeHudBridge(context);
const React=require('react'),R=require('react-test-renderer'),THREE=require('three');
const RC=require('../src/rendering/firstPerson/runtimeController.ts'),TC=require('../src/rendering/firstPerson/theatreController.ts'),D=require('../src/domain/theatre/definition.ts'),L=require('../src/domain/theatre/lightGate.ts'),Defaults=require('../src/types/application.ts');
const {MOVE_SPEED}=require('../src/domain/firstPerson/index.ts'),AI=require('../src/domain/theatre/actor.ts'),{endPointer,beginStick}=require('../src/rendering/firstPerson/touchInput.ts'),Ames=require('../src/domain/theatre/perspectiveExhibit.ts');
const {TheatreScene}=require('../src/rendering/firstPerson/TheatreScene.tsx'),{createSceneResources}=require('../src/rendering/firstPerson/resources.ts');
...
async function extract(){
 ...
 const frames=[],hudTrees=[],lookup=new Map(),previous=new Map(),events=[],samples=[];let priorShadow='',railBeforeCommit,segment='initial',ticks=0,maxCameraMatrixDelta=0,capturing=true,prepTicks=0,lastPhase='',maxActorRadius=0,minActorY=Infinity,maxActorY=-Infinity,actorVertexSamples=0,completedCallbackCount=0;
 ...
 async function step(dt=1/60){
  ...
  const phase=c.runtime.theatre.actor.phase;
  if(capturing){phaseDurations[phase]=(phaseDurations[phase]||0)+dt;if(phase!==lastPhase){event('actor-phase',{from:lastPhase,to:phase,position:c.runtime.theatre.actor.motion.position});lastPhase=phase;}
 ...
 async function solve(){lookAt(D.THEATRE_LIGHT_FIXTURE.center);await hud.update();await hud.pressTestID('interact');if(c.runtime.theatre.mode!=='light')throw Error('Enter light failed');await touch('Start',0);await touch('Move',.8);await touch('End',.8);await press('灯りを固定する');if(!c.runtime.progress.theatre.light.accepted)throw Error('Solve failed');await press('探索へ戻る');event('light-explicitly-solved');await wait(.5);}
 ...
 if(scenario==='light')for(let n=0;n<1200;n++){
  ...
  if(ticks===390){await press('灯りを固定する');if(c.runtime.progress.theatre.light.accepted)throw Error('Wrong released value cleared');event('wrong-retains-closed-gate');}
  ...
  if(ticks===870){if(!await press('灯りを固定する')||!c.runtime.progress.theatre.light.accepted)throw Error('Correct explicit commit failed');event('accepted-after-explicit-commit');segment='physical-gate-opening';}
  if(ticks===930){await press('探索へ戻る');segment='returned-to-explore';}

  await step();
 }else{
  if(scenario.startsWith('projector')||scenario==='curtain-portrait')capturing=false;
