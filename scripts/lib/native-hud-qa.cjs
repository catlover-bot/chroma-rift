...
function auditNavigationHUD(root){const node=id=>root.querySelector('[data-testid="'+id+'"]'),rect=id=>{const e=node(id);return e?hudRect(e):null;};const pause=rect('pause-control'),objective=rect('current-objective'),context=rect('target-context'),movement=rect('movement-stick'),notice=rect('current-notice'),action=rect('interact');return{pause,objective,context,movement,notice,action,objectiveText:node('current-objective')?.textContent,contextText:node('target-context')?.textContent,pauseOverlapsObjective:rectanglesOverlap(pause,objective),contextOverlapsObjective:rectanglesOverlap(context,objective),contextOverlapsMovement:rectanglesOverlap(context,movement),noticeOverlapsObjective:rectanglesOverlap(notice,objective),noticeOverlapsContext:rectanglesOverlap(notice,context),contextOverlapsAction:rectanglesOverlap(context,action)};}
`;
